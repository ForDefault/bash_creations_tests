...
for id in $mass_storage_list; do
    ...
    if [[ -n "$bus_info" ]]; then
        ...
        if [[ -n "$vm_match" ]]; then
            ...
            # Step 6: Check if the device is mounted
            mount_point=$(grep "$bus_info" /etc/mtab | awk '{print $2}')
            if [[ -n "$mount_point" ]]; then
                # If mounted, output mount info instead of VM info
                echo "Mass Storage Device $id" >> "$stopped_vm_list"
                echo "Mounted at: $mount_point" >> "$stopped_vm_list"

                # Step 7: Define a substitute command for the mount path
                echo "mount_path.$id=\$($mount_point)" >> substitute_commands.txt
            else
                echo "No matching VM or mount point found for Mass Storage Device $id" >> "$stopped_vm_list"
            fi
        fi

        # Step 8: Extract and format USB device info into usb_devices_storage.txt
...
# Step 9: Combine the running and stopped VMs and display the result
cat "$running_vm_list" > "$vm_usb_list"
cat "$stopped_vm_list" >> "$vm_usb_list"

# Step 10: Display the generated vm_usb_list.txt and usb_devices_storage.txt
